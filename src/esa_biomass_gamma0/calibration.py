...
def parse_annotation(annotation_xml: bytes | Path) -> CalibrationMetadata:
    ...
    document = annotation_xml.read_bytes() if isinstance(annotation_xml, Path) else annotation_xml
    try:
...
def read_lut_coordinates(lut_path: Path) -> LutCoordinates:
    ...
    with Dataset(lut_path) as dataset:
        ...
        expected_dimensions = (azimuth_variable.dimensions[0], range_variable.dimensions[0])
        if gamma_nought.dimensions != expected_dimensions:
            raise ValueError("radiometry LUT: gammaNought dimensions must be (azimuth, range)")
        shape = tuple(int(size) for size in gamma_nought.shape)
        if shape != (azimuth.size, slant_range.size):
            raise ValueError("radiometry LUT: gammaNought shape does not match coordinate vectors")
...
def lut_pixel_coordinates(
    ...
    return (
        np.interp(azimuth, coordinates.azimuth, np.arange(coordinates.azimuth.size, dtype="float64")),
        np.interp(
            slant_range,
...
def resample_gamma_nought(
    ...
    rows, columns = np.broadcast_arrays(lut_rows[:, np.newaxis], lut_columns[np.newaxis, :])
...
def _coordinate_vector(values: np.ndarray, name: str) -> np.ndarray:
    ...
    if not np.isfinite(values).all() or not np.all(np.diff(values) > 0):
        raise ValueError(f"radiometry LUT: {name} must be finite and strictly monotonic")
    return values
